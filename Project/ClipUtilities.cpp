...
bool ClipUtilities::SameSign(const float a, const float b) {
  return a >= 0 && b >= 0 || a <= 0 && b <= 0;
}

bool ClipUtilities::IsPointOnSameSide(const Vector2& edgePoint1,
...
bool ClipUtilities::isCollinear(const Vector2& a, const Vector2& b,
                                const Vector2& c) {
  return std::fabs(VectorUtilities::Cross(a - b, a - c) == 0);
}

bool ClipUtilities::fuzzyIsCollinear(const Vector2& a, const Vector2& b,
                                     const Vector2& c) {
  return (c.x <= std::max(a.x, b.x) && c.x >= (std::min)(a.x, b.x) &&
          c.y <= std::max(a.y, b.y) && c.y >= (std::min)(a.y, b.y));
}

Vector2 ClipUtilities::PointToLineSegment(const Vector2& a, const Vector2& b,
                                          const Vector2& p) {
  // special cases
  if (a == b) return {};
...
Vector2 ClipUtilities::lineIntersection(const Vector2& p1, const Vector2& p2,
                                        const Vector2& q1, const Vector2& q2) {
  const float d = (p1.x - p2.x) * (q1.y - q2.y) - (p1.y - p2.y) * (q1.x - q2.x);
...
std::vector<Vector2> ClipUtilities::GetVertices(
...
ClipEdge ClipUtilities::FindClipEdge(const std::span<Vector2>& vertices,
                                     size_t index, const Vector2& normal) {
  ClipEdge edge1, edge2;
  edge1.p2 = vertices[index];
  edge2.p1 = vertices[index];
  if (index == 0) {
    edge1.p1 = vertices[vertices.size() - 1];
  ...
  if (std::fabs(VectorUtilities::Dot(edge1.p2 - edge1.p1, normal)) >=
    ...
    finalEdge = edge2;
    Vector2 p =
        VectorUtilities::Right(VectorUtilities::Normalize(edge2.p2 - edge2.p1));
    finalEdge.normal =
        IsPointOnSameSide(edge2.p1, edge2.p2, edge1.p1, edge2.p1 + p) ? p : -p;
  } else {
    finalEdge = edge1;
    Vector2 p =
        VectorUtilities::Right(VectorUtilities::Normalize(edge1.p2 - edge1.p1));
    finalEdge.normal =
        IsPointOnSameSide(edge1.p1, edge1.p2, edge2.p2, edge1.p1 + p) ? p : -p;
  }
  return finalEdge;
}

ClipEdge ClipUtilities::GetClipEdge(const CollisionComponent& shape,
...
std::vector<PointPair> ClipUtilities::Clip(const ClipEdge& clipEdgeA,
